...
| 이름·태그 검색 | 상단 검색바로 **이미 등록된** 상호 찾기·핀 이동 | Should |
| 태그 한/영 함께 쓰기 | Halal, Vegetarian처럼 영어로도 보이게 한다 | Should |
| 지도/필터 링크 공유 | 단톡에 같은 화면 링크를 보낸다 | Could |
...
|------|-------------|-------------------|
| A. 지도+필터 | 동네 지도, 태그 칩, 핀들, 주의 문구, 로그인/로그아웃 | 태그 누르면 핀이 줄고 / 핀 누르면 B로 / “등록” 누르면 (미로그인 시 D 후) C로 |
| B. 상세 패널 | 이름·주소·태그·메모·올린 사람·마지막 수정 (수정은 로그인 유저, 삭제는 본인만) | 닫으면 A로 / 수정·삭제 시 미로그인이면 D / 저장·삭제 후 A |
...
| 실시간 대기·리뷰 긁어오기·길찾기 | 핵심(태그 지도)과 무관하고 범위가 커짐 |
| 완벽한 다국어 앱 | 태그 영문 병기 정도만 하고 UI 전체 번역은 미룸 |
...
| 수정 권한 | **로그인 유저 개방** — 작성자 아니어도 **로그인한 사람**은 태그·메모 수정 가능 (상호·주소·좌표·placeId는 카카오 원본 유지). 비로그인은 보기만 |
...
| 필터 기본 | **칩 선택 없음 = 전체 핀 표시**. 축별로 고른 뒤에만 AND/OR 적용 |
| 주소 저장 | 카카오 **도로명(`road_address_name`) 우선**, 없으면 지번(`address_name`) |
| 상단 검색바 (Must) | **등록용 카카오 장소 검색 입구** — 탭하면 로그인 게이트 후 화면 C. (등록된 상호 필터 검색은 Should) |
...
| 입력 제한 | 메모 0~300자 (빈 메모 허용, DB CHECK). 닉네임 입력란 없음 |
| 태그 칩 표시 | 식단은 `채식 · Vegetarian`처럼 한/영 병기. 음식·유형은 한글만 |
| 상단 검색바 | Must: 등록(C) 입구. Should: 등록된 상호 검색·핀 이동 |
...
| 상단 플로팅 검색바 | 흰 라운드 바 + 그림자. placeholder `장소 검색...`. **Must: 탭 → 등록(C)** (미로그인 시 D). Should: 등록된 상호 필터 |
...
- 면책: 하단 중앙 또는 패널 하단 작은 텍스트. 지도를 가리지 않게 반투명 바 가능
...
구성: **지도 풀블리드 + 플로팅 UI** (레퍼런스). 등록·상세·로그인은 오버레이. (공유 링크 Could는 나중에 `?spot=` 쿼리)

```
┌─────────────────────────────────────────────────────────┐
│  [🔍 장소 검색...]              [필터▾] [로그인/아바타]   │  ← 플로팅
│  [채식][비건][할랄] …  (칩 한 줄, 넘치면 가로 스크롤)     │
│                                                         │
│                     카카오맵 (100%)                       │
│                        · 핀들                            │
│                                                         │
│  [12곳]                                      [＋ 등록]   │  ← 건수 / FAB
│  공식 인증 아님 · 방문 전 매장 확인                        │  ← 면책
│                              ┌──────────────────────┐   │
│                              │ 상호 · 태그 · 메모    │   │  ← 데스크톱
│                              │ 원등록 / 수정         │   │     우측 패널
│                              │ [수정] [삭제]    [×] │   │
...
모바일: 우측 패널 대신 **하단 바텀시트**. 필터 칩은 검색바 아래 한 줄.

### 9.1 화면 A — 지도 + 플로팅 셸

- 풀스크린 카카오맵, 초기 중심 동국대, 줌 ~레벨 4~5
- **상단 검색바**: 흰 라운드 플로팅. **Must = 등록 입구** (탭 → 미로그인이면 D → C에서 카카오 장소 검색). Should로 등록된 상호 하이라이트·이동을 같은 바에 붙일 수 있음
- **필터 칩**: 검색바 바로 아래. **선택 없음 = 전체 표시**. 식단 항상 노출, 음식·유형은 “더보기”
- **우상단**: 로그인 / 아바타+로그아웃 (레퍼런스의 작은 원형 버튼 자리)
- **좌하단**: `N곳` 건수 뱃지 (필터 적용 시 `N / M곳`)
- **우하단 FAB**: “등록” — 미로그인이면 D
- **하단 면책** 1줄: `공식 할랄·채식 인증이 아닙니다. 방문 전 매장에 확인해 주세요.`
- 필터 0개: 토스트 `조건에 맞는 곳이 없어요. 태그를 줄여 보세요.`
...
- 비로그인에서 수정·삭제 → D 후 복귀
- 수정 모드: 같은 패널 안에서 태그·메모 편집. 식단 0개면 저장 비활성
- **영업시간·국문명·외부 링크 섹션은 만들지 않음** (레퍼런스에 있어도 데이터/범위 밖)
...
| `memo` | `text` | default `''`, **CHECK length ≤ 300** | |
...
`place_id`, `name`, `address`, `lat`, `lng`, `created_by`, `created_by_nickname`, `source`

앱은 태그·메모·`last_edited_by`·`last_edited_nickname`·`updated_at`만 보낸다.  
`memo` 길이 ≤ 300, 태그 값은 프리셋 화이트리스트 — CHECK 또는 트리거로 검증.
...
| 3 | 닉네임 입력란 | **없음** |
| 4 | 수정 범위 | **태그·메모만**. 상호·좌표·placeId·원등록자·source는 불변 |
| 5 | 컬럼 보호 | 앱만 믿지 않음 → **BEFORE UPDATE 트리거**로 불변 컬럼 변경 거부 |
| 6 | 식단 태그 | 등록·수정 모두 `diet_tags` **≥ 1** |
| 7 | 중복 등록 | `place_id` UNIQUE |
| 8 | 필터 기본 | **칩 없음 = 전체 표시**. 축 안 OR · 축 사이 AND |
| 9 | 상단 검색바 (Must) | **등록(C) 입구**. 등록된 상호 검색은 Should |
| 10 | 주소 | **도로명 우선**, 없으면 지번. `lat`←`y`, `lng`←`x` |
| 11 | 마지막 수정 표시 | **Must** (이름 + 날짜) |
| 12 | 상세 UI | 데스크톱 우측 패널 · 모바일 바텀시트 |
| 13 | 핀 색 | **딥 그린** `#1B7A4E` 단일 |
| 14 | 내 위치 버튼 | 1차 **제외** (동국대 고정 중심) |
| 15 | 배포 | **Vercel** |
| 16 | UI 언어 | 한글 UI + 식단 태그만 한/영 병기 |
| 17 | OAuth returnTo | **sessionStorage** (`register` / `edit:{id}`) |
| 18 | 지도 | 카카오맵 (레퍼런스 Google Maps UI만 참고) |
| 19 | Anonymous / 이메일가입 / 카카오로그인 | **안 함** |
| 20 | 시드 | 본인 전화·방문 확인분만. AI는 후보만 |
...
| C | Should: 등록 상호 검색을 1차에 넣을지 | **주 3에 시간 남으면** | 없어도 Must 완주 가능 |
| D | 필터 URL 공유 (`?diet=halal`) | **1차 제외** | Could |
